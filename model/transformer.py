...
@torch.no_grad()
def sample_with_past(model, x, steps, temperature=1., sample_logits=True,
    ...
    for n in range(steps):
        if callback is not None:
            callback(n)
        if cbox is None:
            logits, present = model.forward_with_past(idx = x, past=past, past_length=(n+cond_len-1))
        else:
            logits, present = model.forward_with_past(idx = x, past=past, past_length=(n+cond_len-1), cbox=cbox)
        if past is None:
            past = [present]
        else:
            past.append(present)
